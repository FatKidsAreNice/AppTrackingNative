...
it('defines local as the default jobs data source in config', function () {
    expect(file_get_contents(config_path('coldstore.php')))
        ...
        ->toContain("env('COLDSTORE_JOBS_DEFAULT_SCRAP_PERCENT', 10)")
        ->toContain("env('COLDSTORE_JOBS_KSK_CSV_PATH', storage_path('app/coldstore/ksk-values.csv'))")
...
it('uses a jobs view state that replaces the overview with the selected detail view', function () {
    ...
    expect($js)
        ...
        ->toContain('${renderMatchingUids(detail.matchingUids, detail.order?.va_menge_kg, detail.order?.ksk_percent)}')
        ...
        ->toContain('const DEFAULT_CABINET_SCRAP_PERCENT = 10;')
        ->toContain('function formatPercent(percentValue)')
        ->toContain('function normalizeScrapPercent(scrapPercent)')
        ->toContain('function buildCabinetWeightBreakdown(netWeightKg, orderWeightKg, scrapPercent = null)')
        ->toContain('const normalizedScrapPercent = normalizeScrapPercent(scrapPercent);')
        ->toContain('const scrapWeightKg = normalizedNetWeightKg * (normalizedScrapPercent / 100);')
        ...
        ->toContain("const scrapLabel = weightBreakdown?.usesDefaultScrapPercent ? 'Verschnitt Standard' : 'Verschnitt KSK';")
        ->toContain('<small>${escapeHtml(scrapLabel)} (${escapeHtml(formatPercent(weightBreakdown.scrapPercent))}): ${escapeHtml(formatCabinetWeight(weightBreakdown.scrapWeightKg))}</small>')
